...
# Regular Encoder for Stick-Breaking Latent Variables with Gauss-Logit distribution (Logit SB-VAE)
class StickBreakingEncoder_Logit(object):
    def __init__(self, rng, input, batch_size, in_size, latent_size, W_mu = None, W_sigma = None, epsilon = 0.01):
        self.srng = theano.tensor.shared_randomstreams.RandomStreams(rng.randint(999999))
        self.input = input
        ...
        # compute Logit samples
        uniform_samples = T.cast(self.srng.uniform(size=(batch_size, latent_size), low=0.01, high=0.99), theano.config.floatX)
        self.mu = T.dot(self.input, self.W_mu)
        self.log_sigma = T.dot(self.input, self.W_sigma)
        v_samples = T.log(uniform_samples/(1 - uniform_samples))*self.log_sigma + self.mu
...
# Regular Encoder for Stick-Breaking Latent Variables with Gauss-Logit distribution (Logit SB-VAE)
class StickBreakingEncoder_Gamma_VAE(object):
    def __init__(self, rng, input, batch_size, in_size, latent_size, W_a = None, W_b = None, epsilon = 0.01):
        self.srng = theano.tensor.shared_randomstreams.RandomStreams(rng.randint(999999))
        self.input = input
        ...
        self.a = Softplus(T.dot(self.input, self.W_a))
        self.b = Softplus(T.dot(self.input, self.W_b))
        v_samples = ((uniform_samples*self.a*T.gamma(self.a))**(1/self.a))/self.b
    ...
    def calc_kl_divergence(self, prior_alpha, prior_beta):
        ...
        psi_a_taylor_approx = T.log(self.a) - 1./(2 * self.a) - 1./(12 * self.a**2)
        kl = (self.a-prior_alpha)*psi_a_taylor_approx
        kl += - T.gammaln(self.a) + T.gammaln(prior_alpha) + prior_alpha*(T.log(self.b) - T.log(prior_beta)) + ((self.a*(prior_beta - self.b))/self.b)
        return kl.sum(axis=1)
